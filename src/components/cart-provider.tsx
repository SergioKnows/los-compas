...
export function CartProvider({children}:{children:ReactNode}){
 ...
 return <Context.Provider value={{lines,ready,open,setOpen,count:lines.reduce((n,l)=>n+l.quantity,0),add,quantity:(key,n)=>setLines(old=>n<=0?old.filter(l=>l.key!==key):old.map(l=>l.key===key?{...l,quantity:Math.min(Math.max(Math.floor(n),1),20)}:l)),clear:()=>setLines([]),notice}}>{children}<div role="status" aria-live="polite" className={`pointer-events-none fixed bottom-[90px] left-1/2 z-[90] max-w-[calc(100%-40px)] -translate-x-1/2 whitespace-nowrap rounded-md bg-carbon px-5 py-3.5 text-[13px] text-white shadow-[0_4px_18px_#0002] transition-[opacity,transform] duration-200 max-md:w-max max-md:whitespace-normal max-md:text-center max-md:text-xs ${notice?'translate-y-0 opacity-100':'translate-y-[15px] opacity-0'}`}>{notice}</div></Context.Provider>;
}
