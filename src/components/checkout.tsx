...
import {ArrowLeft,ArrowRight,ShoppingBag,MapPin,MessageCircle,ShieldCheck,Check,Copy} from 'lucide-react';
import {useCart} from './cart-provider';
import {CartLines,WhatsAppIcon} from './store-shell';
import {business,money,subtotal,orderMessage,whatsappUrl,type Customer,type PaymentMethod} from '@/lib/catalog';
...
// Cada copia lleva su propio estado, para que el ✓ solo aparezca en el campo que se tocó.
function CopyField({label,value}:{label:string;value:string}){
 const [copied,setCopied]=useState(false);
 const copy=async()=>{try{await navigator.clipboard.writeText(value);setCopied(true);setTimeout(()=>setCopied(false),2500);}catch{}};
 return <button type="button" className="mt-2 flex w-full max-w-[220px] items-center justify-between gap-3 rounded-[5px] border border-line bg-soft p-3 text-left" onClick={copy}><span className="min-w-0"><span className="block text-[10px] text-muted">{label}</span><span className="font-mono text-sm">{value}</span></span>{copied?<Check size={16} className="shrink-0"/>:<Copy size={16} className="shrink-0"/>}</button>;
}
function PaymentCard({method}:{method:PaymentMethod}){
 return <div className="flex flex-wrap items-start gap-6 max-sm:flex-col max-sm:items-center">
  <a href={method.qr} download className="block shrink-0 overflow-hidden rounded-lg border border-line shadow-[0_2px_8px_#0001]" aria-label={`Descargar código QR de ${method.name}`}>
   <Image src={method.qr} alt={`Código QR para transferir a ${method.name}`} width={180} height={180}/>
  </a>
  <div className="max-sm:flex max-sm:flex-col max-sm:items-center">
   <p className="text-sm font-bold">{method.name}</p>
   <p className="text-xs text-muted">{method.holder}</p>
   {method.account&&<CopyField label={method.detail?`Cuenta - ${method.detail}`:'Cuenta'} value={method.account}/>}
   <CopyField label="Llave" value={method.key}/>
  </div>
 </div>;
}
export function Checkout(){
 const cart=useCart(),[customer,setCustomer]=useState<Customer>({name:'',phone:'',address:'',reference:'',notes:'',payment:'A convenir por WhatsApp'}),[error,setError]=useState(''),[prepared,setPrepared]=useState(false);
 ...
 return <main id="contenido" className={`${wrap} pt-[35px] pb-[75px] max-md:pt-5 max-md:pb-[45px]`}>
  ...
  {!cart.ready?<div role="status" className={empty}>Cargando tu pedido…</div>:cart.lines.length?<div className="grid grid-cols-[1.25fr_.85fr] items-start gap-[75px] md:max-lg:grid-cols-[1.1fr_.9fr] md:max-lg:gap-[30px] max-md:grid-cols-1 max-md:gap-[35px]">
   <form className="min-w-0" onSubmit={submit}>
    <div className="grid grid-cols-2 gap-[18px] max-md:grid-cols-1 max-md:gap-0"><label className={label} htmlFor="name">Tu nombre<span className="text-tomate" aria-hidden="true"> *</span><input className={field} id="name" name="name" autoComplete="name" required maxLength={80} value={customer.name} onChange={e=>update('name',e.target.value)} placeholder="Nombre y apellido"/></label><label className={label} htmlFor="phone">Celular<span className="text-tomate" aria-hidden="true"> *</span><input className={field} id="phone" name="phone" type="tel" inputMode="tel" autoComplete="tel" required maxLength={22} value={customer.phone} onChange={e=>update('phone',e.target.value)} placeholder="300 123 4567"/></label></div>
    <label className={label} htmlFor="address">Dirección de entrega<span className="text-tomate" aria-hidden="true"> *</span><input className={field} id="address" name="address" autoComplete="street-address" required maxLength={180} value={customer.address} onChange={e=>update('address',e.target.value)} placeholder="Calle, número, barrio y municipio"/></label>
    ...
    <label className={`${label} mt-6`} htmlFor="notes">¿Algo que debamos saber? <span className={optional}>(opcional)</span><textarea className={field} id="notes" name="notes" rows={3} maxLength={500} value={customer.notes} onChange={e=>update('notes',e.target.value)} placeholder="Sin cebolla, indicaciones de entrega…"/></label>
    ...
    {prepared&&<div role="status" className="mt-[15px] flex gap-2.5 rounded-[5px] bg-[#eef5ed] p-4 text-[13px] text-hoja"><Check size={19} className="shrink-0"/><div><strong>Tu mensaje está listo.</strong><p className="mt-1 text-xs leading-relaxed">Envíalo en WhatsApp para que podamos confirmar tu pedido. Si no se abrió, vuelve a pulsar el botón. Tu carrito sigue guardado.</p></div></div>}   </form>
  ...
  {business.paymentMethods.length>0&&<section className="mt-[65px] border-t border-line pt-[38px] max-md:mt-10 max-md:text-center">
   <div className={`${eyebrow} max-md:justify-center`}>Todo claro, compa</div>
   <h2 className="my-3 text-[25px] font-bold">Sobre tu pago</h2>
   <div className="mt-6 flex flex-wrap gap-x-10 gap-y-10 max-sm:justify-center [&>*]:min-w-[280px]">
    {business.paymentMethods.map(m=><PaymentCard key={m.name} method={m}/>)}
   </div>
   <p className="mt-6 max-w-[440px] text-[11px] leading-relaxed text-muted max-md:mx-auto">No transfieras antes de que confirmemos disponibilidad, domicilio y total por WhatsApp.</p>
  </section>}
 </main>;
}
