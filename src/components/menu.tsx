...
import {wrap,eyebrow,textButton,button,overlay,dialog,dialogClose,dialogTitle,dialogText,productNumber,tone} from './ui';
function ProductCard({product,index}:{product:Product;index:number}){
 ...
 return <article className="flex flex-col overflow-hidden rounded-lg border border-line transition-[box-shadow,transform] duration-[250ms] motion-safe:hover:-translate-y-[3px] hover:shadow-[0_12px_32px_#25252508]">
  ...
  <div className="flex-1 px-7 pt-7 pb-[18px] max-md:px-[22px] max-md:pt-[23px] max-md:pb-[15px]">
   ...
   <div className="mt-6 mb-2.5 max-md:mt-[19px]"><h4 className="mb-2 text-xs font-bold">¿Qué lleva?</h4><p className="text-[13px] leading-[1.85] text-[#66665f]">{product.ingredients.join(' · ')}</p></div>
   <Dialog.Root open={open} onOpenChange={setOpen}>
    <Dialog.Trigger className={textButton}>Ingredientes y salsas <ArrowUpRight size={16}/></Dialog.Trigger>
    <Dialog.Portal><Dialog.Overlay className={overlay}/><Dialog.Content className={dialog}>
     ...
     <ul className="list-disc pl-[18px] text-[13px] leading-[1.9] text-muted max-md:text-xs">{product.ingredients.map(i=><li key={i}>{i}</li>)}</ul>
  ...
  <div className="mx-7 flex items-center justify-between gap-5 border-t border-line py-[22px] max-md:mx-[22px] max-md:py-[18px]">
   ...
   <button className={`inline-flex min-h-11 items-center gap-[9px] rounded-md px-[17px] py-3 text-[13px] font-bold transition-[transform,background] duration-200 hover:brightness-95 motion-safe:hover:-translate-y-0.5 disabled:opacity-50 ${t.add}`} aria-label={`Agregar ${product.name}`} disabled={!ready} onClick={e=>add(product.id,sauces,e.currentTarget)}><Plus size={21}/><span>Agregar</span></button>
  </div>
 </article>;
}
